...
/* ---------------------------------------------------------------------------
 * L'ALTEZZA DI UNA RIGA HA UNA SOLA FONTE
 *
 * La finestra virtuale moltiplica l'altezza di riga per il numero di righe che
 * NON disegna: se il numero non è quello vero, gli spaziatori dichiarano
 * un'altezza sbagliata e la barra di scorrimento promette un contenuto che non
 * c'è (o ne nasconde una parte). Era però scritto in quattro posti che non si
 * parlavano — `QUERY_ROW_H = 36` nella tab ⚡, `ALTEZZA_RIGA_RIQUADRO = 34`
 * nella Split-View, un `28` di ripiego in `grid.js`, e il valore VERO prodotto
 * dal CSS, che non coincideva con nessuno dei tre. Cambiare il padding di una
 * cella li falsificava tutti in silenzio.
 *
 * Ora la fonte è il token `--grid-row-h`, cioè lo stesso numero che il CSS usa
 * per imporre l'altezza: non possono divergere perché sono lo stesso valore.
 * Si legge dall'elemento e non da `:root` perché un tema personalizzato può
 * ridefinire il token su un sottoalbero.
 * ------------------------------------------------------------------------- */
export const ALTEZZA_RIGA_RIPIEGO = 30;

export function altezzaRigaGriglia(el) {
  const base = el && el.nodeType === 1 ? el : document.documentElement;
  const grezzo = getComputedStyle(base).getPropertyValue('--grid-row-h');
  const n = parseFloat(grezzo);
  // Un token assente o illeggibile non deve produrre `NaN`: con `NaN`
  // l'aritmetica della finestra restituisce una finestra vuota, cioè una
  // griglia che non disegna nulla.
  return Number.isFinite(n) && n > 0 ? Math.round(n) : ALTEZZA_RIGA_RIPIEGO;
}
