...
- `public/js/` — frontend modulare (vanilla JS, nessuna build, nessun framework). Stato in un singolo oggetto `state.js`; utility condivise in `utils.js`. L'entrypoint è `main.js`, che importa i moduli specifici: `connection.js`, `dbtree.js`, `schema-ops.js`, `grid.js`, `inlineEdit.js`, `insert.js`, `details.js`, `live.js`, `socket.js` e `uml.js`. Il workspace ha tre viste (`setView`): **Dati** (griglia), **Dettagli** (statistiche/indici/schema) e **UML** (SVG generato in `uml.js`). Menu contestuale generico (tasto destro su database e collection nella sidebar).
