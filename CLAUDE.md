...
* **Un contenitore che scorre non porta padding VERTICALE, se è virtualizzato
  (`#palette-lista`)**: le frecce nella palette (`Ctrl+P`) potevano portare la
  selezione fuori dalla vista — l'ultima riga raggiunta restava tagliata di
  qualche pixel sul fondo, un difetto preesistente ma raro (misurato: circa
  1 volta su 20). Due cause, non una. La prima è lo **scroll anchoring** del
  browser: `muoviSelezione` scrive `scrollTop` e SUBITO DOPO ridisegna la
  finestra virtuale nella stessa reazione, e il browser — per non far
  "saltare" il contenuto sotto l'occhio — aggiusta lo scroll da sé, sommando
  fino a un'intera riga (34px) a quello già corretto scritto da
  `scorrimentoPerRiga`; si spegne con `overflow-anchor: none`, perché qui la
  posizione di ogni riga è CALCOLATA e non deve mai dipendere da "cosa c'era
  prima". La seconda, più piccola ma sistematica, è un `padding` VERTICALE di
  4px sul contenitore stesso: `scorrimentoPerRiga` (lo stesso modulo della
  griglia dei risultati, dove il contenitore scorrevole non ha mai avuto
  padding) assume che la riga 0 cominci esattamente a `scrollTop: 0`, e un
  padding-top sposta ogni riga quel tanto più in basso di quanto la matematica
  preveda — invisibile ovunque tranne che al bordo, dove allineare l'ULTIMA
  riga al fondo lasciava una fetta della sua altezza fuori dalla vista. Lo
  spazio orizzontale resta (`padding: 0 4px`): tocca solo la larghezza, mai
  l'altezza che il calcolo usa. Le due cause si mascheravano a vicenda — è per
  questo che il difetto era raro anziché sistematico — e sono state isolate
  spegnendole una alla volta, non insieme.
...
* **Split-View (`split-layout.js` / `splitview.js`)**: Struttura ad albero immutabile per affiancare più tabelle/collezioni nello stesso workspace con supporto al trascinamento, il cui ridimensionamento usa Pointer Events senza scatti. **Lo scorrimento ridisegna la finestra, non il riquadro**: passava da `updatePaneUI`, cioè a ogni fotogramma ricostruiva l'intestazione con i suoi ascoltatori e riscorreva `p.docs` quattro volte — colonne dichiarate (`colonneRisultato`), righe con `_id`, insieme degli id visibili, e di nuovo le righe con `_id` per la casella «seleziona tutti» — per riscrivere le venti righe visibili. È lavoro proporzionale ai DATI dentro un ciclo che dipende dai PIXEL scorsi, ed è la ragione per cui scorrere una tabella scattava; scorrendo però non cambia nulla di tutto ciò, cambia solo quali righe stanno in DOM. `updatePaneUI` ripone sul riquadro la chiusura `ridisegnaFinestra` — che chiude sopra le righe, le colonne e le relazioni di adesso — e l'ascoltatore, agganciato una volta sola, chiama quella; con il riquadro in caricamento o vuoto la chiusura viene **azzerata**, altrimenti uno scorrimento cancellerebbe «Nessuna riga trovata» per ridisegnare una pagina che non c'è più. Il difetto non si vedeva dalle righe disegnate, che erano identiche: `test/e2e-griglia-viste.js` lo misura sull'**identità dei nodi `<th>`** dopo uno scorrimento vero, e la sensibilità è stata verificata rimettendo di proposito `updatePaneUI` nell'ascoltatore.
* **Calcoli pesanti su Web Worker (`calcoli.js` / `calcoli-protocollo.js` / `calcoli-worker.js`)**: le statistiche della selezione, la scansione dei campi e il **precalcolo** dei grafici (`precalcola` in `chart-option.js`: raggruppamento, aggregazione, ordinamento) scorrono tutte le righe, e spostarle su un module worker ha DUE costi, non uno: il calcolo e il trasporto. `postMessage` non condivide la memoria, e la copia strutturata la paga il thread **chiamante** — proprio quello che si voleva liberare. Misurato su un result set di 50.000 righe per 12 colonne: precalcolo **16 ms**, copia delle righe **126 ms**; su 200.000 righe, 50 ms contro 628. Il trasporto costa da otto a dodici volte il lavoro che evita e cresce insieme a lui, quindi non esiste una soglia oltre la quale cominci a convenire — e il grafico lo pagava **due volte per disegno** (scansione dei campi e precalcolo) a ogni modifica del pannello: era questo a far scattare l'interfaccia mentre si costruiva un grafico. La regola sta quindi sulla **forma** di ciò che attraversa il confine: un elenco piatto di VALORI (le statistiche della selezione) si copia in meno di un millisecondo ogni 50.000 celle e sopra quella soglia va sul worker; un elenco di RIGHE non ci va mai. `campiDisponibili` ne guarda comunque solo 300 — due millisecondi e mezzo — mentre il peso veniva stimato sull'intero result set: da 4.200 righe in su copiava tutto per non calcolare quasi nulla. Le due vie chiamano lo stesso `eseguiCompito`, quindi non possono divergere; senza `Worker`, o se il worker muore, si ricade sul calcolo locale. Il **disegno** resta sul thread principale: l'option di ECharts contiene funzioni (`formatter`) e non attraversa il confine fra thread. Il ridimensionamento del riquadro ridisegna il grafico ma non cambia i dati: il memo di `precalcolaGraficoAsync` (stesse righe per riferimento, stessa configurazione per contenuto) è ciò che toglie il lavoro vero. Chi ricalcola in continuazione (la barra di stato durante il trascinamento) usa `sequenziatore()` per scartare le risposte sorpassate. `test/unit-calcoli.js` mette un Worker finto che registra ogni `postMessage` e prova che le righe non passino MAI di lì.
* **Custom Charts (`charts.js` / `chart-option.js`)**: Generatore di grafici ECharts 6.1.0 (vendorizzato) con aggregazione lato client, suggerimenti automatici e palette accessibili WCAG. Il caricamento della libreria e la cromatura del tema stanno in `chart-runtime.js`, condivisi con il grafico della selezione. **La libreria si scalda da sola a macchina ferma** (`scaldaEcharts`): ECharts sono **1,1 MB** di JavaScript, e leggerli dal disco non costa quasi nulla (25 ms) mentre compilarli ed eseguirli costa il resto. Misurato aprendo la finestra 📈 su una selezione di 50 celle: **286 ms** la prima volta, **34 ms** tutte le successive — quei 250 ms cadevano fra il clic e la finestra, cioè nel momento peggiore. Caricarli all'avvio rallenterebbe la partenza di chi un grafico non lo apre mai, quindi si aspetta la prima pausa vera (`requestIdleCallback` con un `timeout` che garantisce che la pausa arrivi comunque). Non basta però lo `<script>`: ECharts prepara alla PRIMA `init` + `setOption` un'altra fetta di sé (temi, sistemi di coordinate, il primo canvas), altri 46 ms misurati — il riscaldamento disegna quindi un grafico minimo fuori schermo e lo **smonta subito**, perché un canvas dimenticato lì è memoria e un ResizeObserver per un grafico che nessuno vedrà. Un errore si ingoia di proposito: è un anticipo, non una funzione, e se la libreria manca davvero lo dirà l'apertura del grafico, che ha una finestra dove scriverlo. `test/e2e-avvio-ui.js` prova entrambe le cose, e la sensibilità è stata verificata togliendo l'anticipo e la rimozione del div.
